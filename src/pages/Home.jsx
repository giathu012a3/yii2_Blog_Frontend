...
export default function Home() {
  ...
  const [pagination, setPagination] = useState({ total: 0, page: 1, limit: 6, total_page: 1 });
            ...
            {pagination.total_page > 1 && (
              <div className="mt-12 flex justify-center items-center gap-2">
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-white/[0.03] border border-white/5 text-white/60 hover:text-white hover:bg-white/[0.08] disabled:opacity-40 disabled:hover:bg-white/[0.03] disabled:hover:text-white/60 transition-all font-medium text-sm flex items-center gap-1 cursor-pointer disabled:cursor-not-allowed"
                >
                ...
                {Array.from({ length: pagination.total_page }, (_, i) => i + 1).map(pageNumber => (
                  <button
                    key={pageNumber}
                    ...
                    className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm transition-all cursor-pointer ${
                      currentPage === pageNumber
                        ? 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white shadow-lg shadow-indigo-500/25 border-none'
                ...
                <button
                  onClick={() => setCurrentPage(prev => Math.min(pagination.total_page, prev + 1))}

...
export default function Register() {
  ...
  const validate = () => {
    ...
    if (!form.password_confirmation) {
      errs.password_confirmation = 'Vui lòng xác nhận mật khẩu';
    } else if (form.password !== form.password_confirmation) {
      errs.password_confirmation = 'Mật khẩu xác nhận không khớp';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (responseData && typeof responseData === 'object') {
        ...
        setErrors({ general: 'Đăng ký tài khoản thất bại. Vui lòng thử lại.' });
      }
    } finally {
      setLoading(false);
    }
        ...
        <div style={S.formPanel}>
          <div style={S.formInner}>
            <div style={S.formHead}>
              <h1 style={S.formTitle}>Tạo tài khoản</h1>
              <p style={S.formSub}>
                Đã có tài khoản?{' '}
                <Link to="/login" style={S.inlineLink}>Đăng nhập ngay</Link>
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} noValidate style={S.form}>
              ...
              <div style={S.field}>
                ...
                <div style={{ position: 'relative' }}>
                  <svg style={S.iconLeft} width="16" height="16" viewBox="0 0 24 24" fill="none"
                    stroke={focused === 'password_confirmation' ? '#818cf8' : '#64748b'} strokeWidth="2">
                    <rect x="3" y="11" width="18" height="11" rx="2" ry="2"/>
                    <path d="M7 11V7a5 5 0 0 1 10 0v4"/>
                  </svg>
                  <input
                    ...
                    disabled={loading}
                    style={{ ...inputStyle('password_confirmation'), paddingRight: '3rem' }}
                    onFocus={() => setFocused('password_confirmation')}
                    onBlur={() => setFocused('')}
                  />
                  <button type="button" onClick={() => setShowPassConfirm(p => !p)}
                    style={S.eyeBtn} tabIndex={-1}>
                    {showPassConfirm
                      ? <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/><path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/><line x1="1" y1="1" x2="23" y2="23"/></svg>
                      : <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                ...
                {errors.password_confirmation && <span style={S.errMsg}>{errors.password_confirmation}</span>}
              </div>
